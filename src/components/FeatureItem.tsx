...
const FeatureItem = ({title, description, alignmentClasses}: {title: string, description: string, alignmentClasses?: string}) => {
  return (
    <div className={`${alignmentClasses} bg-[#ffeff6] lg:bg-transparent py-10 px-10 sm:px-40 lg:px-0 rounded-md`}>
      <h2 className="font-bold text-primary text-2xl">{title && title}</h2>
